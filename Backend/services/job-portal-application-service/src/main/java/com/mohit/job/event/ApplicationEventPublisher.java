...
public class ApplicationEventPublisher {
    ...
    public void publishStatusChanged(JobApplication app, ApplicationStatus oldStatus, String note) {
        try {
            ...
            kafkaTemplate.send(TOPIC_STATUS_CHANGED, String.valueOf(app.getId()), event);
    ...
    public void publishNoteAdded(JobApplication app) {
        try {
            UserResponse candidate = userClient.getUserById(app.getCandidateId());
            JobSummaryResponse job = jobClient.getJobSummaryById(app.getJobId());
            CompanySummaryResponse company = companyClient.getCompanySummaryById(app.getCompanyId());
