...
const statusConfig = {
  operational: { dot: "bg-emerald-500", text: "text-emerald-700", pulse: true  },
  degraded:    { dot: "bg-amber-500",   text: "text-amber-700",   pulse: false },
  down:        { dot: "bg-red-500",     text: "text-red-700",     pulse: false },
}
      ...
      <CardContent className="pt-4">
          ...
          {services.map(service => {
            ...
            return (
              <div key={service.name} className="flex items-center gap-3">
                <div className={cn("h-2 w-2 rounded-full shrink-0", config.dot, config.pulse && "animate-pulse")} />
                <span className="flex-1 text-sm font-medium text-slate-700 truncate">{service.name}</span>
                <span className="text-xs text-slate-400 hidden sm:block font-mono">{service.latency}</span>
                <span className={cn("text-xs font-bold tabular-nums", config.text)}>{service.uptime}</span>
              </div>
            )
          })}
        </div>
        ...
        <div className="mt-5 pt-4 border-t border-slate-100">
          ...
          <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
            <div className="h-full bg-gradient-to-r from-emerald-400 to-emerald-500 rounded-full" style={{ width: "99.73%" }} />
          </div>
        </div>
      </CardContent>
    </Card>
