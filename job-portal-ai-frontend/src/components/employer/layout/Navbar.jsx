...
import {
  ...
  Building2, Settings, LogOut, FileText,
  BrainCircuit, CalendarCheck, Megaphone,
} from "lucide-react"
...
const notifications = [
  { id: 1, icon: FileText,      title: "New application received",   desc: "Alex Chen applied for Senior React Developer",         time: "3 min ago",  iconColor: "text-brand",          iconBg: "bg-blue-50"    },
  { id: 2, icon: BrainCircuit,  title: "AI screening complete",      desc: "12 candidates auto-shortlisted for Frontend Engineer",  time: "25 min ago", iconColor: "text-violet-600",     iconBg: "bg-violet-50"  },
  { id: 3, icon: CalendarCheck, title: "Interview scheduled",        desc: "Maria Santos — tomorrow at 11:00 AM",                  time: "1 hr ago",   iconColor: "text-emerald-600",    iconBg: "bg-emerald-50" },
  { id: 4, icon: Megaphone,     title: "Job posting expiring soon",  desc: '"Backend Engineer" expires in 2 days',                 time: "3 hr ago",   iconColor: "text-amber-600",      iconBg: "bg-amber-50"   },
]
...
export default function Navbar({ onMenuClick }) {
  ...
  return (
    <header className="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-4 border-b border-slate-200 bg-white px-4 lg:px-6">
      {/* Mobile hamburger */}
      <Button variant="ghost" size="icon" className="lg:hidden text-slate-600" onClick={onMenuClick}>
      ...
      <div className="flex items-center gap-2 ml-auto">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="gap-2 px-2 hover:bg-slate-100">
              <Avatar className="h-8 w-8">
                {user?.profileImage && <AvatarImage src={user.profileImage} alt={user.fullName ?? user.name} />}
          ...
          <DropdownMenuContent align="end" className="w-56">
            <DropdownMenuLabel>
              <p className="font-semibold text-slate-900">{user?.fullName ?? user?.name ?? "Employer"}</p>
